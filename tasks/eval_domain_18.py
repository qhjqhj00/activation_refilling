...
from src.utils import tok

logger = logging.get_logger(__name__)
prompts = {
  "qa": "You are given a book. You're required to read the book and answer the questions.\n\nNow the book begins. \n\n{context}\n\nNow the book ends.\n\nAnswer the following questions.\n\n{input}"
}
...
def process_longbench(data, indices, tokenizer, max_length=3500, truncate_from_middle=True):
    outputs = {'context': [], 'question': [], "dataset": [], "label": [], "index": [], "length": [], "context_id": []}

    for input, context, dataset, label, context_id, index in zip(data['input'], data['context'], data['label'], data['label'], data["context_id"], indices):
        ...
        if max_length is not None:
            if truncate_from_middle:
                ...
                if len(tokenized_context) < max_length:
                    raw_length = len(tokenized_context)
                    i=1
                    while raw_length < max_length:
                        raw_length += 4096
                        expanded_context = context+" ".join(context.split()[-4096*i:])
                        i+=1
