...
class ACRETrainer(Trainer):
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_stride is not None:
            ...
            return StrideGroupedSampler(
                ...
                window=self.model.memory.config.acre_window,
                stride=self.model.memory.config.acre_stride,
    ...
    def evaluate(self, eval_dataset: Dataset | None = None, ignore_keys: List[str] | None = None, metric_key_prefix: str = "eval") -> Dict[str, float]:        
        ...
        self.model.memory.reset()
        train_l1_l2_ratio = self.model.memory.l1_l2_ratio
        train_l1_l2_ratio_mix = self.model.memory.l1_l2_ratio_mix
        self.model.memory.set(
            l1_l2_ratio=self.args.eval_l1_l2_ratio,
            l1_l2_ratio_mix=self.args.eval_l1_l2_ratio_mix,
        )
        ...
        self.model.memory.reset()
        self.model.memory.set(
            l1_l2_ratio=train_l1_l2_ratio,
            l1_l2_ratio_mix=train_l1_l2_ratio_mix,
        )
